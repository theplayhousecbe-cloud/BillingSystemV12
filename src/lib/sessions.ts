import { storage } from "./storage";
import { MENU_ITEMS, type Person, type PricingPlan, type Session, type TableId } from "./types";
import { supabase } from "./supabase";
import { syncRegistration, syncBill } from "@/server/sheets.functions";
import { computeBill, ensurePersons } from "./billing";

function uid() {
  return Date.now().toString(36) + Math.random().toString(36).slice(2, 8);
}
function nextLabel(existing: Person[]): string {
  const used = new Set(existing.map((p) => p.label));
  for (let i = 0; i < 26; i++) {
    const l = String.fromCharCode(65 + i);
    if (!used.has(l)) return l;
  }
  // overflow: AA, AB…
  for (let i = 0; i < 26; i++)
    for (let j = 0; j < 26; j++) {
      const l = String.fromCharCode(65 + i) + String.fromCharCode(65 + j);
      if (!used.has(l)) return l;
    }
  return "?";
}

function activeCount(persons: Person[]) {
  return persons.filter((p) => !p.leftAt).length;
}

export const sessionsApi = {
  list(): Session[] {
    return storage.getSessions();
  },
  active(): Session[] {
    return storage.getSessions().filter((s) => s.status === "active");
  },
  byTable(t: TableId): Session | undefined {
    return storage.getSessions().find((s) => s.status === "active" && s.tableIds.includes(t));
  },

  create(input: {
    customerName: string;
    customerMobile: string;
    tableIds: TableId[];
    adults: number;
    kids: number;
    kidsAbove10: number;
    pricing: PricingPlan;
    plannedDurationMin: number;
    staffName: string;
    startedAt: number;
    hosts?: string[];
  }): Session {
    // Build persons[] up front from adults + kids + kidsAbove10 + memberRates.
    const persons: Person[] = [];
    const rates = input.pricing.memberRates ?? [];
    const kinds = input.pricing.memberKinds ?? [];
    for (let i = 0; i < input.adults; i++) {
      const rate = rates[i] ?? input.pricing.adultRate;
      const kind = kinds[i] ?? (rate === 0 ? "cafe_only" : rate === 99 ? "games_only" : "adult");
      persons.push({
        id: uid(),
        label: String.fromCharCode(65 + i),
        kind,
        joinedAt: input.startedAt,
        firstHourRate: rate,
      });
    }
    for (let j = 0; j < input.kids; j++) {
      const i = input.adults + j;
      const rate = rates[i] ?? input.pricing.kidRate;
      const kind = kinds[i] ?? (rate === 0 ? "cafe_only" : rate === 99 ? "games_only" : "kid");
      persons.push({
        id: uid(),
        label: String.fromCharCode(65 + i),
        kind,
        joinedAt: input.startedAt,
        firstHourRate: rate,
      });
    }
    for (let j = 0; j < input.kidsAbove10; j++) {
      const i = input.adults + input.kids + j;
      const rate = rates[i] ?? input.pricing.kidRateAbove10;
      const kind = kinds[i] ?? (rate === 0 ? "cafe_only" : rate === 99 ? "games_only" : "kid_above_10");
      persons.push({
        id: uid(),
        label: String.fromCharCode(65 + i),
        kind,
        joinedAt: input.startedAt,
        firstHourRate: rate,
      });
    }

    const s: Session = {
      id: uid(),
      ...input,
      status: "active",
      history: [{ at: input.startedAt, adults: input.adults, kids: input.kids }],
      persons,
    };
    const all = storage.getSessions();
    all.push(s);
    storage.setSessions(all);
    syncRegistration({ session: s }).catch(() => {});
    return s;
  },

  // NEW — add a single person. Joins NOW (or at provided ts). Auto-labelled.
  addPerson(id: string, kind: "adult" | "kid" | "kid_above_10" | "games_only" | "cafe_only" = "adult", joinedAt: number = Date.now()) {
    const all = storage.getSessions();
    const s = all.find((x) => x.id === id);
    if (!s) return;
    const persons = ensurePersons(s).slice();
    const label = nextLabel(persons);
    const firstHourRate = 
      kind === "adult" ? s.pricing.adultRate : 
      kind === "kid_above_10" ? s.pricing.kidRateAbove10 : 
      kind === "games_only" ? 99 :
      kind === "cafe_only" ? 0 :
      s.pricing.kidRate;
    persons.push({ id: uid(), label, kind, joinedAt, firstHourRate });
    s.persons = persons;
    if (kind === "adult") s.adults = persons.filter((p) => !p.leftAt && p.kind === "adult").length;
    else if (kind === "kid_above_10") s.kidsAbove10 = persons.filter((p) => !p.leftAt && p.kind === "kid_above_10").length;
    else if (kind === "kid") s.kids = persons.filter((p) => !p.leftAt && p.kind === "kid").length;
    s.history.push({ at: joinedAt, adults: s.adults, kids: (s.kids ?? 0) + (s.kidsAbove10 ?? 0) });
    storage.setSessions(all);
  },

  // NEW — mark a specific person as left.
  removePerson(id: string, personId: string, leftAt: number = Date.now()) {
    const all = storage.getSessions();
    const s = all.find((x) => x.id === id);
    if (!s) return;
    const persons = ensurePersons(s).slice();
    const p = persons.find((x) => x.id === personId);
    if (!p || p.leftAt) return;
    p.leftAt = leftAt;
    s.persons = persons;
    s.adults = persons.filter((x) => !x.leftAt && x.kind === "adult").length;
    s.kids = persons.filter((x) => !x.leftAt && x.kind === "kid").length;
    s.kidsAbove10 = persons.filter((x) => !x.leftAt && x.kind === "kid_above_10").length;
    s.history.push({ at: leftAt, adults: s.adults, kids: s.kids + (s.kidsAbove10 ?? 0) });
    storage.setSessions(all);
  },

  // NEW — edit the session start/end times. Persons are shifted/clamped.
  updateSessionTimes(id: string, newStart: number, newEnd?: number) {
    const all = storage.getSessions();
    const s = all.find((x) => x.id === id);
    if (!s) return;
    const oldStart = s.startedAt;
    const oldEnd = s.endedAt;
    const persons = ensurePersons(s).slice();
    
    // Threshold for matching timestamps (1 second) to handle millisecond precision loss in UI inputs
    const isSameTime = (t1: number, t2?: number) => t2 !== undefined && Math.abs(t1 - t2) < 1000;

    persons.forEach((p) => {
      // If they joined at (or very close to) the session start, move them to the new start
      if (isSameTime(p.joinedAt, oldStart)) p.joinedAt = newStart;
      // Clamp join time to never be before session start
      if (p.joinedAt < newStart) p.joinedAt = newStart;

      if (newEnd) {
        // If they were capped at the old session end, move them to the new end
        if (isSameTime(p.leftAt ?? -1, oldEnd)) p.leftAt = newEnd;
        // Clamp left time to never be after session end
        if (p.leftAt && p.leftAt > newEnd) p.leftAt = newEnd;
        // Clamp join time to never be after session end
        if (p.joinedAt > newEnd) p.joinedAt = newEnd;
      } else {
        // Switching back to Live: if they were capped at oldEnd, make them active again
        if (isSameTime(p.leftAt ?? -1, oldEnd)) p.leftAt = undefined;
      }
    });
    s.persons = persons;
    s.startedAt = newStart;
    s.history = s.history.map((h) => (isSameTime(h.at, oldStart) ? { ...h, at: newStart } : h));
    s.endedAt = newEnd;
    s.pausedMs = 0;
    persons.forEach((p) => { p.pausedMs = 0; });
    storage.setSessions(all);
    window.dispatchEvent(new Event("ph_sessions_changed"));
  },

  // Legacy — kept for backward compat with old UI; routes to add/remove.
  updatePersons(id: string, adults: number, kids: number, kidsAbove10: number = 0) {
    const all = storage.getSessions();
    const s = all.find((x) => x.id === id);
    if (!s) return;
    const persons = ensurePersons(s).slice();
    const now = Date.now();
    const curAdults = persons.filter((p) => !p.leftAt && p.kind === "adult");
    const curKids = persons.filter((p) => !p.leftAt && p.kind === "kid");
    const curKidsAbove10 = persons.filter((p) => !p.leftAt && p.kind === "kid_above_10");

    // adults
    while (curAdults.length > adults) {
      const last = curAdults.pop()!;
      const ref = persons.find((p) => p.id === last.id)!;
      ref.leftAt = now;
    }
    while (curAdults.length < adults) {
      const label = nextLabel(persons);
      const np: Person = { id: uid(), label, kind: "adult", joinedAt: now, firstHourRate: s.pricing.adultRate };
      persons.push(np); curAdults.push(np);
    }
    // kids below 10
    while (curKids.length > kids) {
      const last = curKids.pop()!;
      const ref = persons.find((p) => p.id === last.id)!;
      ref.leftAt = now;
    }
    while (curKids.length < kids) {
      const label = nextLabel(persons);
      const np: Person = { id: uid(), label, kind: "kid", joinedAt: now, firstHourRate: s.pricing.kidRate };
      persons.push(np); curKids.push(np);
    }
    // kids above 10
    while (curKidsAbove10.length > kidsAbove10) {
      const last = curKidsAbove10.pop()!;
      const ref = persons.find((p) => p.id === last.id)!;
      ref.leftAt = now;
    }
    while (curKidsAbove10.length < kidsAbove10) {
      const label = nextLabel(persons);
      const np: Person = { id: uid(), label, kind: "kid_above_10", joinedAt: now, firstHourRate: s.pricing.kidRateAbove10 };
      persons.push(np); curKidsAbove10.push(np);
    }

    s.persons = persons;
    s.adults = adults; 
    s.kids = kids;
    s.kidsAbove10 = kidsAbove10;
    s.history.push({ at: now, adults, kids: kids + kidsAbove10 });
    storage.setSessions(all);
  },

  updateMenuQty(id: string, itemId: string, qty: number) {
    const all = storage.getSessions();
    const s = all.find((x) => x.id === id);
    if (!s) return;
    const orders = { ...(s.menuOrders ?? {}) };
    if (qty <= 0) delete orders[itemId];
    else orders[itemId] = qty;
    s.menuOrders = orders;
    storage.setSessions(all);
  },
  extendSession(id: string, addMinutes: number = 60) {
    const all = storage.getSessions();
    const s = all.find((x) => x.id === id);
    if (!s) return;
    s.plannedDurationMin = (s.plannedDurationMin ?? 60) + addMinutes;
    storage.setSessions(all);
  },
  addTables(id: string, tables: TableId[]) {
    const all = storage.getSessions();
    const s = all.find((x) => x.id === id);
    if (!s) return;
    const set = new Set([...s.tableIds, ...tables]);
    s.tableIds = Array.from(set);
    storage.setSessions(all);
  },
  removeTable(id: string, table: TableId) {
    const all = storage.getSessions();
    const s = all.find((x) => x.id === id);
    if (!s) return;
    s.tableIds = s.tableIds.filter((t) => t !== table);
    storage.setSessions(all);
  },
  freeTables(): TableId[] {
    const taken = new Set<TableId>();
    storage.getSessions().filter((s) => s.status === "active").forEach((s) => s.tableIds.forEach((t) => taken.add(t)));
    return storage.getTables().filter((t) => !taken.has(t));
  },
  async sendNewOrdersToKitchen(id: string) {
    const all = storage.getSessions();
    const s = all.find((x) => x.id === id);
    if (!s) return;

    const current = s.menuOrders ?? {};
    const sent = s.sentOrders ?? {};
    
    const newItems: Array<{ name: string; qty: number }> = [];
    
    Object.entries(current).forEach(([itemId, qty]) => {
      const alreadySent = sent[itemId] ?? 0;
      if (qty > alreadySent) {
        const item = MENU_ITEMS.find(m => m.id === itemId);
        newItems.push({
          name: item?.label || itemId,
          qty: qty - alreadySent
        });
      }
    });

    if (newItems.length === 0) return;

    // To prevent 409 Conflict, check if an active order exists and append to it
    const { data: existingOrder } = await supabase.from("orders").select("*").eq("session_id", s.id).neq("status", "completed").maybeSingle();

    let err;
    if (existingOrder) {
      const combinedItems = [...(existingOrder.items || []), ...newItems];
      const { error } = await supabase.from("orders").update({
        items: combinedItems,
        updated_at: new Date().toISOString()
      }).eq("id", existingOrder.id);
      err = error;
    } else {
      const { error } = await supabase.from("orders").insert({
        session_id: s.id,
        table_number: s.tableIds.join(", "),
        table_ids: s.tableIds,
        customer_count: (s.persons?.filter(p => !p.leftAt).length) || (s.adults + s.kids),
        items: newItems,
        created_at: new Date().toISOString()
      });
      err = error;
    }

    if (!err) {
      s.sentOrders = { ...current };
      storage.setSessions(all);
      return true;
    } else {
      console.error("Kitchen sync error:", err);
      throw err;
    }
  },
  cancel(id: string) {
    const all = storage.getSessions();
    const s = all.find((x) => x.id === id);
    if (!s) return null;
    
    s.status = "cancelled";
    s.endedAt = Date.now();
    const persons = ensurePersons(s).slice();
    persons.forEach((p) => { if (!p.leftAt) p.leftAt = s.endedAt; });
    s.persons = persons;
    storage.setSessions(all);
    
    supabase.from("orders").update({ 
      status: "completed",
      updated_at: new Date().toISOString()
    }).eq("session_id", s.id).then(({ error }) => {
      if (error) console.error("Failed to cancel session order in supabase:", error);
    });
  },
  complete(id: string) {
    const all = storage.getSessions();
    const s = all.find((x) => x.id === id);
    if (!s) return null;
    
    // Respect manually set end time if present, otherwise use now
    const endedAt = s.endedAt || Date.now();
    
    s.status = "completed";
    s.endedAt = endedAt;
    // close any still-active persons at end time
    const persons = ensurePersons(s).slice();
    persons.forEach((p) => { if (!p.leftAt) p.leftAt = endedAt; });
    s.persons = persons;
    storage.setSessions(all);
    const bill = computeBill(s, endedAt);
    syncBill({ bill, session: s }).catch(() => {});
    return bill;
  },
  updateHosts(id: string, hosts: string[]) {
    const all = storage.getSessions();
    const s = all.find((x) => x.id === id);
    if (!s) return;
    s.hosts = hosts;
    const newHost = hosts[0] || "";
    s.staffName = newHost;
    storage.setSessions(all);
    
    const currentTotalGuests = s.adults + s.kids + (s.kidsAbove10 || 0);
    const tableString = s.tableIds.join(", ");

    // 1. Sync the new host and current counts to Registrations to trigger buzzer/notification
    supabase.from("registrations")
      .update({ 
        staff_name: newHost,
        adults: s.adults,
        kids: s.kids + (s.kidsAbove10 || 0),
        tables: tableString
      })
      .eq("id", id)
      .then(({ error }) => {
        if (error) console.error("Failed to sync host swap to registrations table:", error);
      });

    // 2. Update any active orders to reflect the new table/count for the new host
    supabase.from("orders")
      .update({ 
        table_number: tableString,
        customer_count: currentTotalGuests
      })
      .eq("session_id", id)
      .in("status", ["pending", "accepted", "juice_done", "snack_done"])
      .then(({ error }) => {
        if (error) console.error("Failed to sync host swap to orders table:", error);
      });

    window.dispatchEvent(new Event("ph_sessions_changed"));
  },
};
