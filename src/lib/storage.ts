...
if (typeof window !== "undefined") {
        ...
        data?.forEach((row) => {
          ...
          if (row.data?.list) {
            ...
            if (key === "sessions") {
              ...
              const remoteSessions = row.data.list;
              // Map to easily find and merge based on ID
              const map = new Map();
              remoteSessions.forEach((s: any) => map.set(s.id, s));
              
              let localHasExtra = false;
              localSessions.forEach((ls: any) => {
                const remote = map.get(ls.id);
                if (!remote) {
                  map.set(ls.id, ls);
                  localHasExtra = true;
                } else if ((ls.status === "completed" || ls.status === "cancelled") && remote.status === "active") {
                  // Local is finished but remote thinks it's active - local is newer!
                  map.set(ls.id, ls);
                  localHasExtra = true;
                }
              });
              ...
              localStorage.setItem(`ph_${key}`, JSON.stringify(merged));
              window.dispatchEvent(new CustomEvent(`ph_${key}_changed`));
              
              if (localHasExtra) {
                storage.syncToSupabase(key, merged);
              }
            } else if (!localRaw) {
  ...
  // Clean up any existing channel with this name (fixes Vite HMR errors)
  const existingChannel = supabase.getChannels().find(c => c.topic === "realtime:app_data_all");
  if (existingChannel) {
    ...
    .on(
      ...
      (payload: any) => {
        ...
        if (SYNC_KEYS.includes(id) && newData?.list) {
          if (id === "sessions") {
            const localRaw = localStorage.getItem(`ph_${id}`);
            const localSessions = localRaw ? JSON.parse(localRaw) : [];
            const remoteSessions = newData.list;
            const map = new Map();
            remoteSessions.forEach((s: any) => map.set(s.id, s));
            ...
            const merged = Array.from(map.values());
            localStorage.setItem(`ph_${id}`, JSON.stringify(merged));
            window.dispatchEvent(new CustomEvent(`ph_${id}_changed`));
            
            if (localHasExtra) {
               // Push our extra local sessions up to the cloud!
               storage.syncToSupabase(id, merged);
            }
          } else {
