              ...
              {active.map((s, i) => (
                <Link key={s.id} to="/session/$id" params={{ id: s.id }} className="glass rounded-2xl p-4 transition hover:scale-[1.01] animate-reveal-up" style={{ animationDelay: `${0.6 + i * 0.1}s` }}>
                  ...
                  <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <span className={`tabular-nums ${Date.now() - s.startedAt > (s.plannedDurationMin || 60) * 60_000 ? "text-destructive font-bold" : ""}`}>
                      {Date.now() - s.startedAt > (s.plannedDurationMin || 60) * 60_000 
                        ? `Overtime: ${formatDuration((Date.now() - s.startedAt) - (s.plannedDurationMin || 60) * 60_000)}` 
                        : `${formatDuration(Date.now() - s.startedAt)} elapsed`}
                    </span>
                  </div>
                </Link>
              ))}
