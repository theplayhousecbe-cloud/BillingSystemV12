...
function NewSession() {
  ...
  // Synchronize 'tables' state with 'autoTables' when in autoMode
  useEffect(() => {
    if (autoMode) setTables(autoTables);
  }, [autoMode, autoTables]);

  // When totalPersons changes, optionally drop into autoMode if not customized
  useEffect(() => {
    if (totalPersons > 0 && tables.length === 0) setAutoMode(true);
  }, [totalPersons, tables.length]);
  // Keep memberSelections length in sync with adults + kids
  const countsRef = useRef({ adults, kids, kidsAbove10 });
  useEffect(() => {
    const prevC = { ...countsRef.current };
    if (prevC.adults === adults && prevC.kids === kids && prevC.kidsAbove10 === kidsAbove10) return;
    
    setMemberSelections((prev) => {
      let adultSeg = prev.slice(0, prevC.adults);
      let kidSeg = prev.slice(prevC.adults, prevC.adults + prevC.kids);
      let above10Seg = prev.slice(prevC.adults + prevC.kids);

      if (adults > prevC.adults) adultSeg.push(...Array(adults - prevC.adults).fill("games-food-149"));
      else adultSeg = adultSeg.slice(0, adults);

      if (kids > prevC.kids) kidSeg.push(...Array(kids - prevC.kids).fill("kids-food-99"));
      else kidSeg = kidSeg.slice(0, kids);

      if (kidsAbove10 > prevC.kidsAbove10) above10Seg.push(...Array(kidsAbove10 - prevC.kidsAbove10).fill("kids-above-10-149"));
      else above10Seg = above10Seg.slice(0, kidsAbove10);

      return [...adultSeg, ...kidSeg, ...above10Seg];
    });
    
    countsRef.current = { adults, kids, kidsAbove10 };
  }, [adults, kids, kidsAbove10]);
  ...
  const submit = (e: React.FormEvent) => {
    ...
    // Explicitly determine member kinds based on UI selections
    const memberKinds: import("@/lib/types").Person["kind"][] = memberSelections.map((sel, i) => {
      if (sel === "games-only-99") return "games_only";
      if (sel === "cafe-only") return "cafe_only";
      if (sel === "kids-food-99") return "kid";
      if (sel === "kids-above-10-149") return "kid_above_10";
      // Handle custom pricing or games-food-149 based on their index (row)
      if (i >= adults + kids) return "kid_above_10";
      if (i >= adults) return "kid";
      return "adult";
    });
              ...
              {isManualStart ? (
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (!isNaN(parsed)) setStartedAt(parsed);
                  }}
          ...
          <section className="glass space-y-4 rounded-2xl p-5 lg:col-span-2">
                ...
                return (
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-bold transition-all ${
                      on ? "text-white shadow-lg scale-105" : "bg-white text-slate-900 border-2 border-slate-200 hover:border-primary"
                    }`}
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="bg-white border-2 border-slate-200 flex-1 rounded-xl px-4 py-2 text-sm font-bold outline-none focus:border-primary text-black"
              />
              <button type="button" onClick={addHost} className="bg-slate-900 text-white inline-flex items-center gap-1 rounded-xl px-4 py-2 text-sm font-bold hover:scale-[1.03] transition-all">
          ...
          <section className="glass space-y-4 rounded-2xl p-5 lg:col-span-2">
            ...
            <div className="rounded-2xl border border-border/40 bg-white/40 p-4 shadow-sm backdrop-blur-md">
              ...
              <div className="flex flex-wrap items-center gap-3">
                {PACKAGES.map((p) => (
                  <label key={p.id} className={`flex cursor-pointer items-center gap-3 rounded-full border px-4 py-2 text-[13px] font-semibold transition-all duration-200 ${
                    bulkPreset === p.id 
                      ? "border-transparent text-white shadow-lg scale-[1.02]" 
                      : "border-border/80 bg-white text-slate-900 hover:border-primary hover:shadow-md"
                ...
                <label className={`flex cursor-pointer items-center gap-3 rounded-full border px-4 py-2 text-[13px] font-semibold transition-all duration-200 ${
                  bulkPreset === "custom" 
                    ? "border-transparent text-white shadow-lg scale-[1.02]" 
                    : "border-border/80 bg-white text-slate-900 hover:border-primary hover:shadow-md"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {memberSelections.map((selection, i) => {
                ...
                return (
                  <div key={i} className="flex items-center justify-between gap-3 rounded-2xl bg-white p-3 shadow-md border border-slate-200">
                    <div className="text-sm">
                      <span className="font-bold text-slate-900">{label}</span>
                    </div>
                    <div className="flex flex-1 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
                      {PACKAGES.filter(pkg => {
                        if (pkg.id === "games-only-99" || pkg.id === "cafe-only") return true;
                        if (i < adults && pkg.id === "games-food-149") return true;
                        if (i >= adults && i < adults + kids && pkg.id === "kids-food-99") return true;
                        if (i >= adults + kids && pkg.id === "kids-above-10-149") return true;
                        return false;
                      }).map((pkg) => {
                        let label = `₹${pkg.adultRate}`;
                        if (pkg.id === "cafe-only") label = "Cafe";
                        if (pkg.id === "kids-food-99") label = "Kids <10";
                        if (pkg.id === "kids-above-10-149") label = "Kids >10";
                        ...
                        return (
                          <button
                            ...
                            className={`rounded-full px-3 py-1.5 text-[11px] font-bold transition-all duration-200 ${
                              isActive ? "text-white shadow-md scale-105" : "bg-slate-100 text-slate-600 hover:bg-slate-200 hover:text-black"
                            }`}
                      ...
                      <div className="flex items-center gap-0.5">
                        ...
                        <input
                          ...
                          className="w-14 rounded-full bg-slate-100 px-2 py-1 text-[11px] font-bold outline-none border border-slate-300 focus:border-primary text-black"
                        />
                      </div>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
              {MENU_ITEMS.map((m) => {
                ...
                return (
                  <div
                    key={m.id}
                    className={`flex items-center justify-between gap-2 rounded-xl border-2 p-2.5 transition ${
                      active ? "border-primary bg-primary/5 shadow-md" : "border-slate-200 bg-white text-slate-900 shadow-sm"
                    }`}
                  >
