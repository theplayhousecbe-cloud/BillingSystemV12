...
function SessionPage() {
  ...
  const [swapHost, setSwapHost] = useState(false);
  ...
  const completeSession = async () => {
    ...
    if (bill) {
      ...
      const { data: existingOrder } = await supabase.from("orders").select("*").eq("session_id", session.id).neq("status", "completed").maybeSingle();
      ...
      if (existingOrder) {
        ...
        const { error } = await supabase.from("orders").insert({
          ...
          status: "completed",
          table_ids: session.tableIds,
          table_number: session.tableIds.join(", "),
          customer_count: session.persons?.length || (session.adults + session.kids),
          bill_items: formattedItems,
          bill_total: bill.total,
          bill_final: bill.total,
          bill_discount: 0,
      ...
      if (syncError) {
        console.error("Failed to sync completed session to Supabase:", syncError);
  ...
  const handleAdd = (kind: "adult" | "kid" | "kid_above_10" | "games_only" | "cafe_only") => {
    sessionsApi.addPerson(session.id, kind, now);
    const label = kind === "adult" ? "Adult" : kind === "kid_above_10" ? "Kid (>10y)" : kind === "games_only" ? "Games Only" : kind === "cafe_only" ? "Cafe Only" : "Kid (<10y)";
      ...
      <main className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        <div className="glass-strong mb-6 grid gap-4 rounded-3xl p-6 sm:grid-cols-3">
          <div>
            ...
            <div className="mt-4">
              <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Hosts</div>
              <div className="flex flex-wrap gap-1.5">
                {(session.hosts && session.hosts.length > 0 ? session.hosts : [session.staffName]).map((h) => (
                  <span key={h} className="inline-flex items-center rounded-md bg-muted px-2 py-0.5 text-[10px] font-bold text-muted-foreground">
                    {h}
                  </span>
                ))}
              </div>
          ...
          <div className="sm:text-center">
            ...
            <div className="mt-2 flex justify-center gap-2">
              ...
              <button 
                onClick={() => setSwapHost(true)} 
                className="inline-flex items-center gap-1.5 rounded-full bg-primary px-4 py-1.5 text-xs font-bold text-primary-foreground shadow-md transition hover:scale-105 active:scale-95"
              >
                <Users className="h-3.5 w-3.5" /> Swap Host
              </button>
            </div>
          </div>
          ...
          <section className="glass rounded-2xl p-5 lg:col-span-2">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <div className="ml-auto flex flex-wrap gap-2">
                ...
                <button onClick={() => handleAdd("games_only")} className="inline-flex items-center gap-1.5 rounded-full border border-indigo-500/40 bg-indigo-500/10 px-3 py-1.5 text-[10px] font-semibold text-indigo-600 dark:text-indigo-400 transition hover:bg-indigo-500/20">
                  <UserPlus className="h-3 w-3" /> Games Only (₹99)
                </button>
                <button onClick={() => handleAdd("cafe_only")} className="inline-flex items-center gap-1.5 rounded-full border border-slate-500/40 bg-slate-500/10 px-3 py-1.5 text-[10px] font-semibold text-slate-600 dark:text-slate-400 transition hover:bg-slate-500/20">
                  <UserPlus className="h-3 w-3" /> Cafe Only (₹0)
                </button>
              </div>
            </div>
                    ...
                    <tr key={c.person.id} className="border-t border-border/60">
                      ...
                      <td className="py-2 pr-2 text-xs">
                        <span className={`rounded-full px-2 py-0.5 ${
                          ...
                          c.person.kind === "games_only" ? "bg-indigo-500/15 text-indigo-600 dark:text-indigo-400" :
                          c.person.kind === "cafe_only" ? "bg-slate-500/15 text-slate-600 dark:text-slate-400" :
                          "bg-success/15 text-success"
                        }`}>
                          {c.person.kind === "adult" ? "Adult" : c.person.kind === "kid_above_10" ? "Kid (Above 10)" : c.person.kind === "games_only" ? "Games Only" : c.person.kind === "cafe_only" ? "Cafe Only" : "Kid (Under 10)"} · ₹{c.person.firstHourRate}/hr
                        </span>
                      </td>
          ...
          <section className="glass rounded-2xl p-5 lg:col-span-2">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                <button 
                  onClick={() => {
                    if (window.confirm("Are you sure you want to completely cancel this session? This cannot be undone.")) {
                      sessionsApi.cancel(session.id);
                      nav({ to: "/" });
                    }
                  }}
                  className="inline-flex items-center gap-2 rounded-full border border-rose-500/40 bg-rose-500/10 px-5 py-2.5 text-sm font-bold text-rose-600 hover:bg-rose-500 hover:text-white transition-all shadow-sm"
                >
                  Cancel Session
                </button>
                <button 
                  onClick={() => setSwapHost(true)}
                  className="inline-flex items-center gap-2 rounded-full border border-primary/40 bg-primary/10 px-5 py-2.5 text-sm font-bold text-primary transition hover:bg-primary/20"
                >
                  <Users className="h-4 w-4" /> Change Host
                </button>
                <button 
            ...
            <p className="mt-3 text-xs text-muted-foreground">Use "Change Host" to reassign staff. "End Session" closes the session and calculates the final bill.</p>
          </section>
        </div>
        ...
        {swapHost && (
          <SwapHostModal
            sessionId={session.id}
            currentHosts={session.hosts ?? [session.staffName]}
            onClose={() => setSwapHost(false)}
          />
        )}
      </main>
    </div>
...
function MenuOrders({ session }: { session: Session }) {
  ...
  const sent = session.sentOrders ?? {};
  
  // Calculate total items and amount (for the summary)
            ...
            <div key={m.id} className="glass rounded-xl p-3">
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setQty(m.id, s + Math.max(0, q - 1))} className="grid h-7 w-7 place-items-center rounded-lg hover:bg-muted">−</button>
                  <span className="w-6 text-center font-mono text-sm tabular-nums">{q}</span>
                  <button onClick={() => setQty(m.id, s + q + 1)} className="grid h-7 w-7 place-items-center rounded-lg hover:bg-muted">+</button>
                </div>
              </div>
            </div>
          );
...
function SwapHostModal({ sessionId, currentHosts, onClose }: { sessionId: string; currentHosts: string[]; onClose: () => void }) {
  const hosts = storage.getHosts();
  const availableHosts = hosts.length > 0 ? hosts : DEFAULT_HOSTS;
  const [selected, setSelected] = useState<string[]>(currentHosts);
  ...
  const handleSave = () => {
    if (selected.length === 0) return toast.error("Select at least one host");
    sessionsApi.updateHosts(sessionId, selected);
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-background/70 p-4 backdrop-blur-sm">
      <div className="glass-strong w-full max-w-sm rounded-3xl p-6 shadow-2xl">
        <h3 className="font-display text-xl font-bold">Swap Host</h3>
        <p className="mt-1 text-sm text-muted-foreground">Change the hosts assigned to this session.</p>
        
        <div className="mt-4 space-y-2">
          <label className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Available Hosts</label>
          <div className="flex flex-wrap gap-2">
